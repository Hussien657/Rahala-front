          ...
          >
            ...
            >
              ...
              <div className="font-semibold">
                {t("subscriptions.active", "You have an active subscription")} —{" "}
                {status.plan?.toUpperCase()}
              </div>
              {status.has_verified_badge && (
                <Badge variant="outline" className="ml-2">
                  {t("subscriptions.verified", "Verified badge")}
                </Badge>
              )}
            </div>
            <div
            ...
            >
              ...
              <div className="flex items-center">
                ...
                />
                <span>
                  {t("subscriptions.remaining", "Days remaining")}:{" "}
      ...
      {isError && (
        <div className="text-center">
          <p className="text-red-600 mb-2">
            {t("subscriptions.failed", "Failed to load plans")}
          </p>
          <Button variant="outline" onClick={() => refetch()}>
            {t("subscriptions.retry", "Retry")}
          </Button>
        </div>
      )}
          ...
          {status?.is_active && (
            <div className="absolute top-3 right-3 z-20">
              <Badge variant="secondary">
                {t("subscriptions.alreadyActive", "Active")}
              </Badge>
            </div>
          )}
        </div>
      ...
      <Card className="overflow-hidden hover:shadow-xl transition-shadow h-full flex flex-col">
        ...
        <CardContent className="flex-1 flex flex-col">
          ...
          <ul className="space-y-2 mb-4">
            ...
            >
              <Check className="h-4 w-4 text-green-600 mt-0.5" />
              <span className="text-sm">1 trip post</span>
            </li>
            <li
              className={`flex items-start ${direction === "rtl" ? "space-x-reverse" : ""} space-x-2`}
            >
              <Check className="h-4 w-4 text-green-600 mt-0.5" />
              <span className="text-sm">3 pictures per trip</span>
            </li>
            <li
              className={`flex items-start ${direction === "rtl" ? "space-x-reverse" : ""} space-x-2`}
            >
              <Check className="h-4 w-4 text-green-600 mt-0.5" />
              <span className="text-sm">1 video allowed (max 100MB)</span>
            </li>
          </ul>
        </CardContent>
      </Card>
